...
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900">Buy & Rent Enquiries</h1>
    <p class="text-gray-500 text-sm mt-1">List of customers interested in buying or renting properties.</p>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody>
                @forelse ($buy_rent as $index => $customer)
    ...
    <!-- Pagination -->
    @if ($buy_rent->hasPages())
    <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
        <p class="text-sm text-gray-500 hidden sm:block">
            Showing <span class="font-medium text-gray-900">{{ $buy_rent->firstItem() }}</span> to <span class="font-medium text-gray-900">{{ $buy_rent->lastItem() }}</span> of <span class="font-medium text-gray-900">{{ $buy_rent->total() }}</span> entries
        </p>
        <div class="flex items-center gap-1">
            ...
            @else
                <a href="{{ $buy_rent->previousPageUrl() }}" class="w-9 h-9 flex items-center justify-center rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-colors">
                    <i class="fa-regular fa-chevron-left text-xs"></i>
                </a>
            @endif
            ...
            @if ($buy_rent->hasMorePages())
                <a href="{{ $buy_rent->nextPageUrl() }}" class="w-9 h-9 flex items-center justify-center rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-colors">
